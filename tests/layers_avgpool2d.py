from .utils import *
from neural_network.modules.pooling2d import AvgPool2d
...
class TestMaxPool2dLayer(unittest.TestCase):
    ...
    def _test_forward_backward(self, kernel_size, stride):
        ...
        self.torch_layer = torch.nn.AvgPool2d(kernel_size=kernel_size, stride=stride, padding=0)
        self.tensorflow_layer = tensorflow.keras.layers.AveragePooling2D(pool_size=kernel_size, strides=stride, padding='valid')
        self.tensorflow_layer(to_tensorflow(self.input_data))  # Tensorflow layer needs a forward pass to initialize...
        self.custom_layer = AvgPool2d(kernel_size=kernel_size, stride=stride)
