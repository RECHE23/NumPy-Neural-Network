from .utils import *
from neural_network.modules.pooling1d import MaxPool1d
...
class TestMaxPool1dLayer(unittest.TestCase):
    ...
    def _test_forward_backward(self, kernel_size, stride):
        ...
        self.torch_layer = torch.nn.MaxPool1d(kernel_size=kernel_size, stride=stride, padding=0, dilation=1)
        self.tensorflow_layer = tensorflow.keras.layers.MaxPooling1D(pool_size=kernel_size, strides=stride, padding='valid')
        self.tensorflow_layer(to_tensorflow(self.input_data))  # Tensorflow layer needs a forward pass to initialize...
        self.custom_layer = MaxPool1d(kernel_size=kernel_size, stride=stride)
