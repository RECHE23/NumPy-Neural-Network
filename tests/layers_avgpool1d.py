from .utils import *
from neural_network.modules.pooling1d import AvgPool1d


class TestAvgPool1dLayer(unittest.TestCase):
    ...
    def _test_forward_backward(self, kernel_size, stride):
        ...
        self.torch_layer = torch.nn.AvgPool1d(kernel_size=kernel_size, stride=stride, padding=0)
        self.tensorflow_layer = tensorflow.keras.layers.AveragePooling1D(pool_size=kernel_size, strides=stride, padding='valid')
        self.tensorflow_layer(to_tensorflow(self.input_data))  # Tensorflow layer needs a forward pass to initialize...
        self.custom_layer = AvgPool1d(kernel_size=kernel_size, stride=stride)
        ...
        np.testing.assert_allclose(tensorflow_output_, custom_output_, rtol=1e-7, atol=1e-6, err_msg=f"Failed for kernel_size={kernel_size}, stride={stride}")
