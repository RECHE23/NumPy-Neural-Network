...
@trace()
def binary_cross_entropy(y_true: np.ndarray, y_pred: np.ndarray, prime: bool = False, multioutput: str = 'uniform_average', epsilon: float = 1e-10) -> Union[np.ndarray, float]:
    """
    ...
    if multioutput == 'raw_values':
        return -np.sum(y_true * np.log(y_pred_clipped) + (1 - y_true) * np.log(1 - y_pred_clipped), axis=-1)
